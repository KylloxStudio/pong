...
def update():
    ...
    print(abs(ball.position.x))
    if abs(ball.position.x) > 1:
        if ball.position.x > 0:    
            info_text = Text("player A win. press space to restart", y=-.45)
        else:
            info_text = Text("player B win. press space to restart", y=-.45)
        isGameOver = True
